...
async function deleteAllUsers() {
	try {
			...
			if (user.profileImage) {
				...
				try {
					if (fs.existsSync(imagePath)) {
						...
						imagesCleaned++;
						console.log(`🗑️  Deleted profile image for user: ${user.username}`);
					}
				} catch (error) {
					console.warn(`⚠️  Could not delete image ${user.profileImage}:`, error.message);
				}
			}
		}
		...
		try {
			...
			console.log('⚠️  Could not reset auto-increment counter (this is usually fine).');
		}
